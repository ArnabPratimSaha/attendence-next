...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios({
      url: `${process.env.NEXT_PUBLIC_BACKEND}/user/class`,
      method: 'GET',
      headers: headers
    }).then(res => {
      if (res.status === 200) {
  ...
  const handleClassCreate = async () => {
    try {
      ...
      const res = await axios({
        url: `${process.env.NEXT_PUBLIC_BACKEND}/class/create`,
        method: 'POST',
        data: {
          name
        },
      ...
      if (res.status === 200) {
        return router.push(`/class/${res.data.id}`)
      }
    } catch (error) {
      ...
      <div className='dashboard-fulldiv'>
        ...
        <div className="dashboard-classes">
          ...
          {classData.map((c, i) => <div key={i} className={`dashboard-classcard`}>
            <div className="dashboard-classcard__left" onClick={() => router.push(`/class/${c.id}`)}>
